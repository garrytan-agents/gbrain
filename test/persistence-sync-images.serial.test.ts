...
afterAll(async () => { await withEnv(env, async () => { await disposePersistenceConsumer(engine); await engine.disconnect(); }); rmSync(home, { recursive: true, force: true }); mock.restore(); });
...
async function fixture(files: Record<string, string | Buffer>, subpath?: string) {
...
async function anchor(id: string) { return (await engine.executeRaw<{ last_commit: string | null }>('SELECT last_commit FROM sources WHERE id=$1', [id]))[0].last_commit; }
const check = (name: string, run: () => Promise<void>) => test(name, () => withEnv(env, async () => {
  ...
  try { await run(); expect(network).toBe(0); } finally { provider = undefined; globalThis.fetch = fetch; }
}), 120_000);

check('mixed Markdown and binary images sync provider-free with durable exact base64 bytes', async () => {
  const f = await fixture({ 'a.md': 'A synthetic source observation.\n', 'images/Photo One.PNG': png }), before = calls;
  ...
  expect(await engine.getFile(f.id, 'images/Photo One.PNG')).toMatchObject({ page_id: page!.id, content_hash: sha256(png), size_bytes: png.length });
  ...
  expect(await anchor(f.id)).toBe(f.head);
});
check('image changes, additions, renames and deletions advance only complete sync manifests', async () => {
  const f = await fixture({ 'first.png': png, 'rename.png': png });
  ...
  expect(await anchor(f.id)).toBe(next);
  ...
  expect(await engine.getPage('first.png', { sourceId: f.id })).toBeNull(); expect(await anchor(f.id)).toBe(last);
});
check('nested source-root image identity does not include the surrounding repository prefix', async () => {
  const f = await fixture({ 'wiki/images/Photo.PNG': png, 'outside.png': png }, 'wiki');
  await performManagedSync(engine, f.opts);
...
check('pinned Git image imports preserve dirty or absent working-tree bytes; opt-in imports current bytes', async () => {
...
check('provider preparation receives pinned binary bytes rather than dirty worktree data', async () => {
  const f = await fixture({ 'photo.png': png }); writeFileSync(join(f.root, 'photo.png'), changed);
  provider = async () => {
    expect(lastInputs).toEqual([{ kind: 'image_base64', data: png.toString('base64'), mime: 'image/png' }]);
    expect(await engine.getPage('photo.png', { sourceId: f.id })).toBeNull(); expect(await anchor(f.id)).toBeNull();
  };
...
check('provider failure blocks the image and final anchor; replay retains its UUID and explicit retry replaces only failed work', async () => {
  const f = await fixture({ 'a.md': 'A committed synthetic observation.\n', 'z.png': png });
  ...
  expect(first).toMatchObject({ status: 'blocked_by_failures', filesImported: 1 }); expect(await anchor(f.id)).toBeNull();
  ...
  provider = async () => { expect(await engine.getPage('z.png', { sourceId: f.id })).toBeNull(); expect(await anchor(f.id)).toBeNull(); };
...
check('worktree bytes changing during provider preparation conflict before image publication', async () => {
  const f = await fixture({ 'photo.png': png });
...
check('canonical revision and owner races during image preparation preserve newer state and the anchor', async () => {
  for (const race of ['revision', 'owner']) {
    ...
    expect(result.status === 'blocked_by_failures' || result.status === 'partial').toBe(true); expect(await anchor(f.id)).toBeNull();
...
check('interrupted mixed sync resumes pinned binary target with immutable processing consent', async () => {
  const f = await fixture({ 'a.md': 'Synthetic text preceding a binary image.\n', 'z.png': png });
  const first = await performManagedSync(engine, f.opts, { maxPages: 1, maxMs: 1000 });
  expect(first).toMatchObject({ status: 'partial', filesImported: 1 }); expect(await anchor(f.id)).toBeNull();
...
check('pinned image update cannot roll back a canonical page when current worktree disagrees', async () => {
  const f = await fixture({ 'photo.png': png }); await performManagedSync(engine, f.opts);
  ...
  expect(await anchor(f.id)).toBe(f.head); expect(readFileSync(join(f.root, 'photo.png'))).toEqual(newer);
  expect(await engine.getPage('photo.png', { sourceId: f.id })).toMatchObject({ content_hash: sha256(png) });
});
check('pinned symlink images and malformed UTF-8 text fail closed', async () => {
  const f = await fixture({ 'target.png': png, 'note.md': 'Valid text.\n' });
  ...
  const bad = await fixture({ 'bad.md': Buffer.from([0xff, 0xfe, 0x80]) });
  await expect(performManagedSync(engine, bad.opts)).rejects.toMatchObject({ code: 'invalid_params' }); expect(await anchor(bad.id)).toBeNull();
});
test('frozen content accepts legacy text only and rejects unknown codecs, type mismatches and malformed binary', () => {
...
check('pending image sync resumes the same request UUID and binary intent after owner lock release', async () => {
  const f = await fixture({ 'photo.png': png }), lock = await acquireWorktree((await getWorktreeBinding(engine, f.id))!);
  ...
  try {
    ...
    expect(await anchor(f.id)).toBeNull(); expect(await engine.getPage('photo.png', { sourceId: f.id })).toBeNull();
    ...
    expect((await prepareManagedSyncMutation(engine, accepted, { engine: 'pglite' })).observedRevision).toBeNull();
    for (const [invalid, code] of [
      ...
      await expect(prepareManagedSyncMutation(engine, { ...accepted, intent: { ...accepted.intent, ...invalid } }, { engine: 'pglite' })).rejects.toMatchObject({ code });
    }
    expect(await engine.executeRaw('SELECT intent FROM persistence_requests WHERE request_id=$1::uuid', [id])).toEqual(frozen);
    await lock!.release();
...
check('fresh binary manifests are invisible to legacy cursor lookup while legacy text resumes unchanged', async () => {
  const f = await fixture({ 'a.md': 'First original text.\n', 'b.md': 'Second original text.\n' });
  const first = await performManagedSync(engine, f.opts, { maxPages: 1, maxMs: 1000 });
...
check('a completed versioned cursor cannot shadow a pending legacy UUID; dual unfinished keys refuse', async () => {
  const f = await fixture({ 'a.md': 'An initial committed observation.\n' });
  await performManagedSync(engine, f.opts);
...
check('explicit idle retry upgrades a legacy unknown-consent cursor without rewriting its manifest', async () => {
  const f = await fixture({ 'a.md': 'Legacy source text.\n' });
  await performManagedSync(engine, f.opts);
